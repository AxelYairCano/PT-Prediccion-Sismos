...
def extraer_datos_de_archivo(ruta_archivo):
    """Extrae datos sísmicos - VERSIÓN CORREGIDA"""
    try:
...
def construir_catalogo_desde_carpeta(root_dir):
    """Construye catálogo con validaciones mejoradas"""
...
def eliminar_sismos_duplicados(df, ventana_segundos=None, delta_mag=None):
    """Elimina duplicados con parámetros configurables"""
...
def generar_vectores_entrenamiento(df):
    ...
    if len(df_filtrado) < start_idx + 1:
        print("⚠️ Insuficientes sismos para entrenamiento")
...
# ==========================================
# 5. EJECUCIÓN PRINCIPAL (ACTUALIZADA)
# ==========================================
if __name__ == "__main__":
    ...
    if len(X_train) > 0:
        ...
        # NUEVA: Crear visualizaciones
        crear_visualizaciones(df_catalogo, X_train, y_train, fechas_train)
    else:
        print("⚠️ No se generaron vectores de entrenamiento")
